...
@conductor_system.route("/list-systems", methods=["GET"])
def list_systems_by_locale():
    ...
    parameters:
      - name: locale
        ...
        type: string
        required: true
        description: Locale name to filter by (e.g. "Penang OpenDC")
    responses:
      200:
...
@conductor_system.route("/systems", methods=["GET"])
def list_systems_by_filter():
    ...
    parameters:
      - name: locale
        in: query
        type: string
        description: Locale to fetch from (default "Penang OpenDC")
      - name: site
        in: query
        type: string
    ...
    responses:
      ...
      504:
        description: Conductor API timed out
    """
    locale_name = request.args.get("locale", "Penang OpenDC").strip()
...
@conductor_system.route("/system/raw-data", methods=["GET"])
def get_system_data():
